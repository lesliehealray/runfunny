...
Runfunny is deployed on the following stack.
...
As documented you should create/import your Celery app in ``runfunny/__init__.py`` so that you
can run the worker via::

    celery -A runfunny worker

Additionally you will need to uncomment the ``BROKER_URL`` setting in the project settings::

    # runfunny/settings/deploy.py
    from .base import *

    # ...
    BROKER_URL = 'amqp://runfunny_%(ENVIRONMENT)s:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/runfunny_%(ENVIRONMENT)s' % os.environ
...
the broker by default. The RabbitMQ user will be named ``runfunny_<environment>`` and the
vhost will be named ``runfunny_<environment>`` for each environment. It requires that you
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out runfunny.key 2048
  # Make copy of the key with the passphrase
  cp runfunny.key runfunny.key.secure
  # Remove any passphrase
  openssl rsa -in runfunny.secure -out runfunny.key
  # Generate signing request
  openssl req -nodes -sha256 -new -key runfunny.key -out runfunny.csr
